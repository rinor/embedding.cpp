...
void embedding_test(bert_ctx *ctx, const std::string &input, const std::vector<bert_vocab_id> &expected)
{
    std::vector<float> embeddings(bert_n_embd(ctx));
    bert_encode(ctx, 1, input.data(), embeddings.data());
